...
* **Cookie tray**: Depending on the chip's shape, a slot will be filled. Each container depends on the number of corners the shape has. Score 6 hexagons, 5 pentagons, 4 squares and 3 triangles in total to fill the tray. Doing so grants a special shape pack.
* **Pillbox**: Fill the pillbox with 10 pills (one of each number). Each chip played with a different number fills the empty slots of the pillbox. Once completed it resets and you'll gain a random consumable.
