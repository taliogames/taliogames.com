...
* **Bosses**: Every 4 rounds you'll fight a boss. Each boss applies a debuff only during its respective round. After defeating 2 normal bosses (red frame) you'll fight a special one (gold frame), which usually has stronger effects that impact gameplay more. Under certain conditions (high pocket deepness) it's possible to fight 2 bosses at once. In that case it alternates between 2 normal bosses or 1 normal boss + 1 special boss (two mana-removing bosses will never appear together). To unlock the Boss Preview Feature, you must win/advance beyond round 12 in 50 runs. It helps you preview the bosses you'll have to beat during your run. For normal deepness levels, you'll only be able to see the special bosses. For D4, you can see all double bosses. It only works until you finish a normal run (it doesn't show Endless Mode bosses). You can check this by clicking the blue 'i' button next to the start run button in the Main Menu, or by clicking on the boss's image during the run. The boss order and their appearances change once the player reaches at least round 3.
...
* **Scoring order**: Combos (Number/Shape/Color) > Chips > Mods > Cards > Pink tube > Special combo.
...
* **Cards**: There are 5 rarities (Common, Rare, Epic, Supreme, Forbidden). They usually execute from left to right, but each card can act at different moments of the run, for example when chips trigger, when entering the shop, when using an item, etc. Cards have 4 traits: Action, cards that execute in order after mods; Unsellable (can't be liquified either); Indestructible (prevents being destroyed by killer cards); Unique (you can only have one); Sinister (doesn't take up slots). By default, you have 12 slots for your cards, with 6 unlocked from the start. The rest can be unlocked in the store for $4, $7, $10, $13, $16, and $20, respectively.
* **Mods**: Equipped on cards and used to gain advantages. Each card rarity has a default number of mod slots (4/3/2/2/0). There are 3 mod rarities: Common (1 slot), Rare (1 slot), Pro (3 slots). They can be unequipped and transferred between cards only on the shop screen. They trigger from left to right. Aside from being able to change them individually by clicking on each mod and each card, you can use the 3 automated management buttons: (1) 'O' button lets you unselect all mods from the clicked cards. (2) 'X' button lets you unselect all mods from all cards at once; you don't lose any mods, the leftover ones in your inventory will go into a queue. (3) 'A' button automatically equips all mods in the most convenient order, prioritizing the highest rarity ones, and equipping the ones that multiply values at the very end.
...
* **Pre-Ban Cards**: you can unlock this feature after you unlock 35 cards. In the codex, you'll be able to select (depending on how many cards you've unlocked) a certain amount of cards of each rarity so they don't show up during your runs. They won't appear in any way at all: shop, packs, rerolls, etc. It's a great feature to force or aim for a specific build ahead of time.